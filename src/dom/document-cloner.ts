...
export class DocumentCloner {
    ...
    toIFrame(ownerDocument: Document, windowSize: Bounds): Promise<HTMLIFrameElement> {
        ...
        if (isSafari) {
            // In safari browser, when a html element (with complicated css) is converted to canvas, the image is not clear post
            // conversion, as if the canvas is drawn without CSS styles.
            // This issue doesn't happen in any other browser. Safari's caching issue.
            // This code will enforce the browser to fetch CSS freshly everytime the iframe is rendered.
